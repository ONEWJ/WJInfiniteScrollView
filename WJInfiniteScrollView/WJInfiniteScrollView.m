...
/*
 ...
 - (void)layoutSubviews{
     ...
     CGFloat X0 = self.scrollView.frame.size.width + 30 - W0;
     ...
     for (int i = 0; i<ImageViewCount; i++) {
         ...
         imageView.frame = CGRectMake(15 + self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width - 30, self.scrollView.frame.size.height);
         
         }else if(i == 2){
         imageView.frame = CGRectMake(2*self.scrollView.frame.size.width - 30, 15, W0, H0);
     ...
     [self updateContent];
     [self.scrollView bringSubviewToFront:self.scrollView.subviews[1]];
 }
 */
